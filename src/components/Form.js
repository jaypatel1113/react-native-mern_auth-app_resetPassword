...
const Form = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if(password.trim().length<8 || password.trim().length>20) return setError("Password must be 8 to 20 char long!");
        if(password !== confirmPass) return setError("Password does not match!");
